...
export const LineGrid = (props) => {
  ...
  const [solidColor] = useState('#CCCCCC70'); // 实线颜色
  const [dashedColor] = useState('#CCCCCC25'); // 虚线颜色
  const [zeroColor] = useState('#358bf3'); // 0 点颜色
  const [strokeWidth] = useState(1); // strokeWidth
  const [shadowOpacity] = useState(0); // shadowOpacity
  const [shadowEnabled] = useState(false); // shadowEnabled
  ...
  gridComponents.push(<Rect x={0} y={0} width={10} height={10} fill="red" />);
